...
const STATUS_STYLES: Record<string, string> = {
  ...
  partially_paid: "bg-secondary text-secondary-foreground",
...
export function InvoicesPage() {
  ...
  const columns: ColumnDef<InvoiceRead>[] = [
    ...
    { accessorKey: "balance_due", header: "Saldo" },
  ];
    ...
    <div>
      <div className="mb-6 flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Facturas de venta y de proveedor. Borrador → Contabilizada → Pago parcial/Pagada.</p>
        </div>
        <CreateInvoiceDialog />
      </div>
      ...
      {!isLoading && !error && rows.length === 0 && <p className="text-sm text-muted-foreground">Todavía no hay facturas.</p>}
      ...
      <InvoiceDetailDialog invoiceId={selectedId} onOpenChange={(open) => !open && setSelectedId(null)} />
    </div>
  );
}
